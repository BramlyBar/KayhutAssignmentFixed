...
def main():
    ...
    if given_group.isdigit():
        try:
            ...
            main_job(int(given_group), given_url, given_key)  # added a call to main so we start the first job now
            sche.add_job(lambda: main_job(int(given_group), given_url, given_key), 'interval', hours=12)  # added lambda
            sche.start()
            return True
        except:
            return False
